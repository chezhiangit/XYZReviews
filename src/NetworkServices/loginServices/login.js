import RNFetchBlob from '../../../node_modules/rn-fetch-blob';
...
const getProvidersService = async () => {
  try {
    const url = urlConstants.BaseUrl + urlConstants.getProviders;
    const headersParams = {};
    headersParams['Content-Type'] = 'application/json';
    console.log('getProvidersService url ...', url);
    console.log('getProvidersService headersParams ...', headersParams);
    ...
    console.log('getProvidersService response ...', response);
    const result = response.json();
    if (response.respInfo.status === 200) {
      result.status = 200;
      return result;
...
export {authenticateUserService, registerNewUserService, getProvidersService};
